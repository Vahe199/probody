...
router.get('/:workerId', apicache.middleware('15 minutes'), async (req, res) => {
    ...
    res.json({
        reviews: await Review.find({target: req.params.workerId}, ['name', 'interior', 'massage', 'service', 'avg', 'text', 'dateCreated']).limit(req.query.limit).skip((req.query.page - 1) * req.query.limit).sort({createdAt: -1}),
