...
router.patch('/:uuid/editandapprove', async (req, res) => {
    try {
        ...
        mongoDoc.validate(async (err) => {
            ...
            await mongoDoc.save()
            await Search.addWorker('search:workers:', mongoDoc._id, doc.name, doc.phone, doc.lastRaise, doc.rooms, doc.description, doc.leads, doc.services, doc.massageTypes, (await Region.findById(doc.region)).name)
            await RedisHelper.unlink(redisKey)
